...
const BASE_PATH = "./circuits/CircuitNisha/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/CircuitNisha.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/CircuitNishaVerifier.sol:Verifier");
